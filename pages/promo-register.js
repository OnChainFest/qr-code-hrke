...
export default function PromoRegister() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const resp = await fetch('/api/promo-register', {
        method: 'POST',
        ...
        <form onSubmit={onSubmit} style={{display:'grid', gap:12}}>
          ...
          <label style={{display:'grid', gap:6}}>
            ...
            <input
              ...
              placeholder="tu@correo.com" style={inputStyle} required
            />
          </label>
          ...
          {status==='ok' && <div style={{marginTop:4, background:'rgba(0,128,0,0.2)', padding:10, borderRadius:8}}>{msg}</div>}
          {status==='error' && <div style={{marginTop:4, background:'rgba(255,0,0,0.25)', padding:10, borderRadius:8}}>{msg}</div>}
        </form>
...
const inputStyle = {
  padding:'12px 14px', borderRadius:10, border:'1px solid rgba(255,255,255,0.3)',
  background:'rgba(255,255,255,0.1)', color:'#fff', outline:'none'
};
const tagLabel = { fontSize:12, opacity:.8, marginBottom:4 };
const tagValue = {
  padding:'8px 10px', borderRadius:8, border:'1px dashed rgba(255,255,255,0.35)',
